export default function AvatarName({ color, initials, name, company, preview = false }: { color: string; initials: string; name: string; company?:string; preview?:boolean }) {
  ...
  return (
    <div className={`flex items-center ${preview && 'max-vsm:justify-center'} gap-2 vsm:gap-3`}>
        ...
        <div className="">
            <span className={`${preview ? `max-vsm:hidden` : ''} ${preview ? '' :'text-ink'}`}>{name}</span>
